...
echo ""
echo "# 2. 运行测试（推荐方式 - 使用环境变量）："
echo "sudo docker run --rm -v /tmp:/host-tmp:Z \\"
echo "  -e S3_BUCKET_NAME=your-actual-bucket-name \\"
echo "  -e AWS_REGION=cn-north-1 \\"
echo "  s3-test:latest"
echo ""
echo "# 3. 或者使用命令行参数方式："
echo "sudo docker run --rm -v /tmp:/host-tmp:Z \\"
echo "  s3-test:latest \\"
echo "  your-actual-bucket-name cn-north-1"
echo ""
echo "# 4. 查看下载的文件："
echo "ls -la /tmp/download/"
echo "cat /tmp/download/downloaded-docker-test-*.txt"
echo ""
echo "注意：请将 'your-actual-bucket-name' 替换为您的实际 S3 桶名"
echo "     请将 'cn-north-1' 替换为您的实际 AWS 区域"
echo "=========================================="
